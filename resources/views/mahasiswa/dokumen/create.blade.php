                ...
                <form action="{{ route('mahasiswa.dokumen.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul"
                            name="judul" placeholder="Masukkan judul dokumen" value="{{ old('judul') }}" required>
                        @error('judul')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('jenis_dokumen') is-invalid @enderror" id="jenis_dokumen"
                            ...
                            <option value="Skripsi" {{ old('jenis_dokumen') == 'Skripsi' ? 'selected' : '' }}>Skripsi
                            </option>
                    ...
                    @if ($role === 'Mahasiswa')
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="nim" name="nim"
                                value="{{ Auth::user()->mahasiswa->nim ?? '-' }}" readonly>
                            <small class="form-text text-muted">NIM diambil otomatis dari data profil Anda</small>
                        </div>
                    @elseif ($role === 'Dosen')
                        <div class="mb-3">
                            <label for="nim" class="form-label">NIDN <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="nim" name="nim"
                                value="{{ Auth::user()->dosen->nidn ?? '-' }}" disabled readonly>
                            <small class="form-text text-muted">NIDN diambil otomatis dari data profil Anda</small>
                        </div>
                    @elseif($role === 'Admin' || $role === 'Operator')
                        <div class="mb-3">
                            ...
                            @error('nim')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                            <small class="form-text text-muted">Masukkan NIM mahasiswa</small>
                        </div>
                    @endif
