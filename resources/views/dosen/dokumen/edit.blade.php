            ...
            <form action="{{ route('dokumen.update', $dokumen->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul"
                        placeholder="Masukkan judul dokumen" value="{{ old('judul', $dokumen->judul) }}" required>
                    @error('judul')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('jenis_dokumen') is-invalid @enderror" id="jenis_dokumen"
                        ...
                        <option value="Skripsi" {{ old('jenis_dokumen', $dokumen->jenis_dokumen) == 'Skripsi' ?
                            'selected' : '' }}>Skripsi</option>
                        <option value="Jurnal" {{ old('jenis_dokumen', $dokumen->jenis_dokumen) == 'Jurnal' ? 'selected'
                            : '' }}>Jurnal</option>
                        <option value="Proposal" {{ old('jenis_dokumen', $dokumen->jenis_dokumen) == 'Proposal' ?
                            'selected' : '' }}>Proposal</option>
                        <option value="KTI" {{ old('jenis_dokumen', $dokumen->jenis_dokumen) == 'KTI' ? 'selected' : ''
                            }}>KTI</option>
                    </select>
                    @error('jenis_dokumen')
                    <span class="invalid-feedback">{{ $message }}</span>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nim') is-invalid @enderror" id="nim" name="nim"
                        placeholder="Masukkan NIM" value="{{ old('nim', $dokumen->nim) }}" required>
                    @error('nim')
                    <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror" id="status" name="status"
                        ...
                        <option value="Di Proses" {{ old('status', $dokumen->status) == 'Di Proses' ? 'selected' : ''
                            }}>Di Proses</option>
